...
const NORMAL_PROMPT =
  ...
  '请严格以 JSON 格式输出（所有调味品用量一律用克 g 或毫升 ml，不要写“适量/少许/勺”）：' +
  '{"name": "正常的菜名，不超过10个字",' +
  '"steps": ["步骤1", "步骤2", "步骤3"],' +
  '"plating": "简单实用的摆盘建议",' +
  '"warning": "一句贴心的注意事项",' +
...
const SYSTEM_PROMPT =
  ...
  '请严格以 JSON 格式输出，不要包含任何多余的解释文本（所有调味品用量一律用克 g 或毫升 ml，不要写“适量/少许/勺”）。' +
  'JSON 结构如下：' +
...
const SYNTH_PROMPT =
  ...
  '请严格以 JSON 格式输出，不要输出多余文字（调味品用量一律用克 g 或毫升 ml）：' +
